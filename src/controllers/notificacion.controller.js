...
async function noLeidas(req, res) {
  try {
    ...
    logger.error('Error al contar notificaciones: ' + error.message, {
      stack: error.stack,
      usuario_id: req.usuario.id,
    });
...
async function marcarLeida(req, res) {
  try {
    ...
    logger.error('Error al marcar notificación: ' + error.message, {
      stack: error.stack,
...
async function marcarTodasLeidas(req, res) {
  try {
    ...
    logger.error('Error al marcar todas las notificaciones: ' + error.message, {
      stack: error.stack,
      usuario_id: req.usuario.id,
    });
